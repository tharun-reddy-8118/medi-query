...
Pull out specific pieces of a timestamp or get the friendly name of a month.

```sql
extract('year' FROM "Date")         -- Returns 2024
extract('month' FROM "Date")        -- Returns 10 (as a number)
extract('day' FROM "Date")          -- Returns 15
extract('isodow' FROM "Date")       -- Returns Day of Week (1 = Monday, 7 = Sunday)

-- Getting the Month Name
monthname("Date")                   -- Returns full name (e.g., 'October')
strftime("Date", '%b')              -- Returns short name (e.g., 'Oct', 'Jan', 'Feb')
```
...
### Example 4: Creating a Date Range Filter Flag
You want to create a flag (True/False or Yes/No) to check if a date falls within a specific date range. This is extremely useful for filtering dashboards to specific promotional or operational periods.

```sql
-- Using standard comparison operators
CASE 
  WHEN "Admission Date" >= '2024-01-01' AND "Admission Date" <= '2024-12-31' THEN 'Within Range'
  ELSE 'Out of Range'
END

-- Alternatively, comparing against other columns:
CASE 
  WHEN "Admission Date" >= "Start Date" AND "Admission Date" <= "End Date" THEN 'True'
  ELSE 'False'
END

-- Or using the BETWEEN operator for cleaner syntax:
CASE 
  WHEN "Admission Date" BETWEEN "Start Date" AND "End Date" THEN 'Yes'
  ELSE 'No'
END
```

### Example 5: Filtering a DateTime by a Single Exact Date
If your column contains both a date AND a time (e.g., `2024-10-15 14:30:25`), directly comparing it to `'2024-10-15'` will fail because the times don't match. You must first convert the column to just a date using `CAST`:

```sql
CASE 
  WHEN CAST("Admission Time" AS DATE) = '2024-10-15' THEN 'Target Day'
  ELSE 'Other Day'
END

-- You can also use the shorthand :: operator for casting
CASE 
  WHEN "Admission Time"::DATE = '2024-10-15' THEN 'Target Day'
  ELSE 'Other Day'
END
```
